...
مثال: انصلح عيب باحتساب الخصم. الـ Sanity Testing بيغطّي الخصم بحالاته (نسبة، مبلغ ثابت، صفر، أكبر من السلة) ومنطقة الحساب حواليه — بلا ما يلمس تسجيل الدخول والإشعارات.
...
| نتيجة الفشل | ترفض النسخة | ترجّع العيب مفتوح | تفتح عيوب انحدار جديدة |

## مثال: إصدار واحد يمشي على الثلاثة

**السياق**: بوابة ويب، الإصدار 2.4، فيه ميزة جديدة «تصدير التقرير PDF» + إصلاح لعيب بالبحث.
...
2. **Sanity** (الساعة 10:40): العيب المُصلَح بالبحث — بحث بكلمة عربية، بكلمة إنجليزية، بنص كلمة، بحقل فاضي، بمسافات زايدة. ✅ الإصلاح ضبط.
3. **اختبار الميزة الجديدة** (باقي اليوم): تصدير PDF بحالاته — تقرير فاضي، تقرير طويل، أسماء عربية، أحرف خاصّة، تصدير مرّتين بسرعة.
4. **Regression** (اليوم التالي): المسارات اللي ما لمسناها — إنشاء مستخدم، رفع مرفقات، الإشعارات، الطباعة، الصلاحيات. ❌ لقينا إنّ زر «طباعة» صار يفتح صفحة فاضية → عيب انحدار (regression bug)، وهو أخطر أنواع العيوب لأنّه بيمسّ شي كان شغّال.
...
- **Retesting (إعادة الاختبار)**: تنفيذ **نفس** الحالة اللي فشلت، على النسخة اللي فيها الإصلاح — للتأكّد إنّ العيب انصلح. حالات محدّدة سلفاً، ما بتنفع للأتمتة الأولى.
- **Regression**: تنفيذ حالات **ثانية** ما فشلت، للتأكّد إنّها لسا ناجحة.

بالعملي: العيب انصلح؟ اعمل Retesting للحالة الفاشلة، وبعدها Regression للمسارات المرتبطة.
...
2. **أضف كل عيب انحدار سابق**: أي عيب رجع مرّتين، حالة اختباره بتصير عضو دائم بالـ Suite.
...
- **إهمال Regression بعد ترقية بيئة أو مكتبة**. كثير عيوب انحدار بتيجي من هون، مش من كود الفريق.
